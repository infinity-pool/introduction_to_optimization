# 2018016244 추현욱
# Problem 1-4_func2 _ steepest (optimal step size) gradient descent
...
for k in range(0, MAX_ITER_NUM - 1):    # Get x_(k+1) from x_k
    step_size = funcs.get_steepest_stepsize(funcs.f2, funcs.grad_f2, x_datas[k]) # Set step size as steepest gradient descent
    x_datas[k + 1] = x_datas[k] - step_size * funcs.grad_f2(x_datas[k]) # Calculate x_(k+1) using 'steepest gradient descent algorithm'
...
plt.title("Gradient Descent Algorithm(Problem 1-4, f2, optimal stepsize)") # title
