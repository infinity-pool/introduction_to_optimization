# 2018016244 추현욱
# Problem 1-5_func1 _ x^*_Gradient obtained by diminishing stepsize (a_k = 1/(k + 1))
...
for k1 in range(0, MAX_ITER_NUM - 1):    # Get x_(k+1) from x_k
    ...
    x_datas_grad[k1 + 1] = x_datas_grad[k1] - step_size1 * funcs.grad_f1(x_datas_grad[k1]) # Calculate x_(k+1) using 'gradient descent algorithm' with constant stepsize
...
for k2 in range(0, MAX_ITER_NUM - 1):    # Get x_(k+1) from x_k
    step_size2 = funcs.get_steepest_stepsize(funcs.f1, funcs.grad_f1, x_datas_steepestgrad[k2]) # Set step size as 'steepest gradient descent'
    x_datas_steepestgrad[k2 + 1] = x_datas_steepestgrad[k2] - step_size2 * funcs.grad_f1(x_datas_steepestgrad[k2]) # Calculate x_(k+1) using 'steepest gradient descent algorithm'
...
plt.title("Gradient Descent Algorithm(Problem 1-5, f1, diminishing stepsize)") # title
