# 2018016244 추현욱
# Problem 1-5_func2 _ x^*_Gradient obtained by constant stepsize (a_k = 0.01)
...
for k1 in range(0, MAX_ITER_NUM - 1):    # Get x_(k+1) from x_k
    x_datas_grad[k1 + 1] = x_datas_grad[k1] - STEP_SIZE * funcs.grad_f2(x_datas_grad[k1]) # Calculate x_(k+1) using 'gradient descent algorithm' with constant stepsize
...
for k2 in range(0, MAX_ITER_NUM - 1):    # Get x_(k+1) from x_k
    step_size = funcs.get_steepest_stepsize(funcs.f2, funcs.grad_f2, x_datas_steepestgrad[k2]) # set step size as 'steepest gradient descent'
    x_datas_steepestgrad[k2 + 1] = x_datas_steepestgrad[k2] - step_size * funcs.grad_f2(x_datas_steepestgrad[k2]) # Calculate x_(k+1) using 'steepest gradient descent algorithm'
...
plt.title("Gradient Descent Algorithm(Problem 1-5, f2, const stepsize)") # title
