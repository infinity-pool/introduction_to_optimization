# 2018016244 추현욱
# Problem 1-5_func2 _ x^*_Gradient obtained by diminishing stepsize (a_k = 1/(k + 1))
...
x_star = np.array([-0.2, -3.7]) # x^* obtained by hand
...
f_datas_grad[0] = funcs.f2(x_datas_grad[0])   # Calculate initial function value
x_datas_steepestgrad[0] = np.array([0, 0])       # Set initial point
f_datas_steepestgrad[0] = funcs.f2(x_datas_steepestgrad[0])   # Calculate initial function value
...
for k1 in range(0, MAX_ITER_NUM - 1):    # Get x_(k+1) from x_k
    ...
    x_datas_grad[k1 + 1] = x_datas_grad[k1] - step_size1 * funcs.grad_f2(x_datas_grad[k1]) # Calculate x_(k+1) using 'gradient descent algorithm' with constant stepsize
    f_datas_grad[k1 + 1] = funcs.f2(x_datas_grad[k1 + 1]) # Calculate f(x_(k+1))
    ...
    if ((abs(f_datas_grad[k1 + 1] - f_datas_grad[k1]) < 1e-6) and np.linalg.norm(funcs.grad_f2(x_datas_grad[k1])) < 1e-6):
        break

for k2 in range(0, MAX_ITER_NUM - 1):    # Get x_(k+1) from x_k
    step_size2 = funcs.get_steepest_stepsize(funcs.f2, funcs.grad_f2, x_datas_steepestgrad[k2]) # set step size as 'steepest gradient descent'
    x_datas_steepestgrad[k2 + 1] = x_datas_steepestgrad[k2] - step_size2 * funcs.grad_f2(x_datas_steepestgrad[k2]) # Calculate x_(k+1) using 'steepest gradient descent algorithm'
    f_datas_steepestgrad[k2 + 1] = funcs.f2(x_datas_steepestgrad[k2 + 1]) # Calculate f(x_(k+1))
    norms2[k2 + 1] = np.linalg.norm(x_star - x_datas_steepestgrad[k2 + 1]) # Calculatate ∣∣x^* - x^*_SteepestGradient∣∣

    # Break when ∣f(x_(k+1)) - f(x_k))∣ < 10^-6 and ∣∣grad(x_k)∣∣ < 10^-6
    if ((abs(f_datas_steepestgrad[k2 + 1] - f_datas_steepestgrad[k2]) < 1e-6) and np.linalg.norm(funcs.grad_f2(x_datas_steepestgrad[k2])) < 1e-6):
        break
...
plt.title("Gradient Descent Algorithm(Problem 1-5, f2, diminishing stepsize)") # title
